...
def main(argv): 
    ...
    ## check arguments
    if not (fantasy_results_file and output_file and cutoff_file):
        print "Error: At least one of the required inputs is missing."
        help()

    driver(fantasy_results_file, output_file, cutoff_file)


def driver(fantasy_results_file, output_file, cutoff_file):
    ...
    standings_dict = get_standings_dict(fantasy_results_df, cutoff_list)

    with open(output_file, 'w') as out_file:
        json.dump(standings_dict, out_file)


def get_standings_dict(fantasy_results_df, cutoff_list):
    ...
    for ind, row in fantasy_results_df.iterrows():
        ...
        try:
            ...
            team_dict[row.team_1] = [0, 0, 0, 0, team_1_goals, team_2_goals, team_1_goals - team_2_goals]
        ...
        try:
            ...
            team_dict[row.team_2] = [0, 0, 0, 0, team_2_goals, team_1_goals,team_2_goals - team_1_goals]
